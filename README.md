...
- 核心技能：用统一卡片展示能力领域和说明。
- 工作经历：时间轴、公司信息、可选公司 Logo、职位、摘要和详情弹窗。
- 旅行足迹：基于真实中国地图轮廓展示城市、路线和地点说明。
- 个人项目：展示 GitHub 仓库、线上地址和项目简介。
- 此刻 NOW：记录近期状态、关注事项、心情、地点、标签和更新时间。
- 我的豆瓣片单：填写豆瓣公开个人主页 URL 后同步影视类的在看与想看记录，可设置每天或每 3、7、14、30 天自动抓取；前台使用“在看 / 想看”Tab，默认显示在看，每类按时间倒序展示最新 8 条，桌面端按每行 4 张显示，全部记录可在无封面列表弹层中查看。
- 照片故事：支持多图、图片说明、排序和灯箱预览。
- 导航：展示前五个已发布模块，其余内容收进“更多”。
- 页面工具：返回顶部、系统分享与复制链接降级。
- 社交媒体标签：支持 GitHub、微信、微博、Telegram、豆瓣、Instagram、YouTube、哔哩哔哩、小红书、知乎、Discord 等常用平台，并可根据链接自动匹配图标。
...
flowchart LR
  ...
  Cron["Vercel Cron（每日检查）"] --> Sync["到期豆瓣模块同步"]
  Sync --> Blob
  Blob --> Public
  Local --> Public
```
...
| `CRON_SECRET` | 自动抓取必需 | 保护豆瓣定时抓取接口的随机密钥 |
...
| 模块 | 主要数据 |
| --- | --- |
| 工作经历 | 公司、公司 Logo、职位、任职时间、摘要、详情 |
| 旅行足迹 | 城市、省份、经纬度、说明、路线 |
| 个人项目 | 名称、简介、GitHub、线上地址 |
| 此刻 NOW | 状态、心情、地点、标签、更新时间 |
| 我的豆瓣片单 | 豆瓣个人主页 URL、自动抓取周期、在看与想看分类、同步时间、封面、评分、短评、外链 |
