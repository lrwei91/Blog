...
export function SpecialModuleForm({
 ...
 return (
 <div className="admin-special-form grid gap-6 text-[#201D18]">
 <section className="grid gap-4 rounded-[6px] border border-[#F4EBE6] bg-[#F4EBE6] p-4">
 <div className="flex items-center gap-3">
 <span className="grid h-10 w-10 place-items-center rounded-[5px] bg-[#FCFAF5] text-[#B23C22]">
 <SpecialModuleIcon type={moduleType} />
 </span>
 <div>
 <h3 className="font-bold text-[#201D18]">
 ...
 <div className="grid gap-3 rounded-[6px] border border-[#DDD6C8] bg-[#FCFAF5] p-4 md:grid-cols-2">
 ...
 <label className="flex items-center gap-2 text-sm font-medium text-[#6F6A5E]">
...
export function SpecialModulePreview({ block }: { block: Block }) {
 ...
 <p className="mt-2 line-clamp-2 max-w-xl text-sm leading-6 text-[#6F6A5E]">{block.description}</p>
 </div>
 ...
 <div className="relative hidden w-[34%] min-w-[180px] items-center justify-center overflow-hidden rounded-[6px] border border-[#EDE8DB] bg-[#FCFAF5]/65 md:flex">
 <div className="absolute inset-0 opacity-45 [background-image:linear-gradient(#E3CFC5_1px,transparent_1px),linear-gradient(90deg,#E3CFC5_1px,transparent_1px)] [background-size:22px_22px]" />
 ...
 key={province.adcode}
 d={province.d}
 fill={locations.some((location) => province.name.includes(location.province)) ? "#B23C22" : "#EDE8DB"}
 stroke="#6F6A5E"
 strokeWidth="1.2"
 />
 ))}
 ...
 <div key={`${index}-${project.title}`} className="min-w-0 rounded-[6px] border border-black/10 bg-[#FCFAF5]/70 p-4">
 <p className="truncate text-[10px] tracking-[0.12em] text-[#6F6A5E]">{project.eyebrow}</p>
 <h3 className="mt-4 line-clamp-2 min-h-12 text-lg font-bold text-[#201D18]">{project.title}</h3>
 <p className="mt-2 line-clamp-2 text-xs leading-5 text-[#6F6A5E]">{project.description}</p>
 </div>
 )) : <div className="md:col-span-3 rounded-[6px] border border-dashed border-[#DDD6C8] py-8 text-center text-sm text-[#6F6A5E]">暂无项目</div>}
 </div>
 </div>
 );
 }
...
function TravelLocationsEditor({
 ...
 <div>
 <h3 className="font-bold text-[#201D18]">{isZh ? "地点列表" : "Locations"}</h3>
 <p className="text-xs text-[#6F6A5E]">{isZh ? "经纬度用于在中国地图上定位标记。" : "Coordinates place each marker on the China map."}</p>
 </div>
 <Button
 type="button"
 variant="secondary"
...
function TravelLocationCard({
 ...
 <span className="grid h-10 w-10 shrink-0 place-items-center rounded-[5px] border border-[#F4EBE6] bg-[#F4EBE6] text-[#B23C22]">
 <MapPin className="h-4 w-4" />
 </span>
 <button type="button" onClick={() => setExpanded((current) => !current)} className="min-w-0 flex-1 text-left">
 <p className="truncate text-sm font-bold text-[#201D18]">{String(index + 1).padStart(2, "0")} · {location.city || (isZh ? "未命名地点" : "Untitled location")}</p>
 <p className="mt-0.5 truncate text-xs text-[#6F6A5E]">{location.province}{location.note ? ` · ${location.note}` : ""}</p>
 </button>
 <div className="flex items-center gap-1">
 <button type="button" disabled={index === 0} onClick={() => onMove(-1)} className="grid h-8 w-8 place-items-center rounded-[5px] text-[#6F6A5E] hover:bg-[#FCFAF5] disabled:opacity-25" title={isZh ? "上移" : "Move up"}><ArrowUp className="h-3.5 w-3.5" /></button>
 <button type="button" disabled={index === length - 1} onClick={() => onMove(1)} className="grid h-8 w-8 place-items-center rounded-[5px] text-[#6F6A5E] hover:bg-[#FCFAF5] disabled:opacity-25" title={isZh ? "下移" : "Move down"}><ArrowDown className="h-3.5 w-3.5" /></button>
 <button type="button" onClick={onDelete} className="grid h-8 w-8 place-items-center rounded-[5px] text-red-500 hover:bg-red-50" title={isZh ? "删除地点" : "Delete location"}><Trash2 className="h-3.5 w-3.5" /></button>
 <button type="button" onClick={() => setExpanded((current) => !current)} className="grid h-8 w-8 place-items-center rounded-[5px] text-[#6F6A5E] hover:bg-[#FCFAF5]" aria-label={expanded ? (isZh ? "收起地点" : "Collapse location") : (isZh ? "展开地点" : "Expand location")}>
 ...
 {expanded ? (
 <div className="grid gap-3 border-t border-[#DDD6C8] bg-[#FCFAF5] p-4 md:grid-cols-2">
...
function PersonalProjectsEditor({
 ...
 <div className="flex items-center justify-between gap-3">
 <div>
 <h3 className="font-bold text-[#201D18]">{isZh ? "项目列表" : "Projects"}</h3>
 <p className="text-xs text-[#6F6A5E]">{isZh ? "GitHub 链接必填，在线地址可选。" : "GitHub URL is required; live URL is optional."}</p>
 </div>
 <Button
 type="button"
 variant="secondary"
 ...
 <article key={`${index}-${project.title}`} className="grid gap-4 rounded-[6px] border border-[#F4EBE6] bg-[#FCFAF5] p-4">
 <ItemToolbar
...
function NowStatusEditor({
 ...
 return (
 <section className="grid gap-4 rounded-[6px] border border-[#F4EBE6] bg-[#FCFAF5] p-4">
 <div>
 <h3 className="font-bold text-[#201D18]">{isZh ? "近况内容" : "Current status"}</h3>
 <p className="text-xs text-[#6F6A5E]">{isZh ? "适合记录近期状态、关注事项和生活关键词。" : "Share a concise current update."}</p>
 </div>
 <div className="grid gap-3 md:grid-cols-2">
...
function PhotoStoriesEditor({
 ...
 <section className="grid gap-4">
 <div className="flex items-center justify-between gap-3">
 <div><h3 className="font-bold text-[#201D18]">{isZh ? "故事列表" : "Stories"}</h3><p className="text-xs text-[#6F6A5E]">{isZh ? "每个故事可以上传多张图片并单独填写说明。" : "Each story can contain multiple captioned photos."}</p></div>
 ...
 <article key={story.id} className="grid gap-4 rounded-[6px] border border-[#F4EBE6] bg-[#FCFAF5] p-4">
 ...
 <div className="grid gap-3 rounded-[6px] bg-[#F4EBE6] p-4">
 <div className="flex items-center justify-between"><div><h4 className="font-bold text-[#201D18]">{isZh ? "照片" : "Photos"}</h4><p className="text-xs text-[#6F6A5E]">{story.photos.length} {isZh ? "张" : "photos"}</p></div></div>
 ...
 <div key={photo.id} className="grid gap-3 rounded-[5px] border border-[#F4EBE6] bg-[#FCFAF5] p-3 md:grid-cols-[88px_1fr]">
 <img src={photo.url} alt="" className="h-[88px] w-[88px] rounded-[5px] object-cover" />
...
function ItemToolbar({
 ...
 const isZh = editorLanguage === "zh-CN";
 return (
 <div className="flex items-center justify-between gap-3 border-b border-[#EDE8DB] pb-3">
 <strong className="min-w-0 truncate text-sm text-[#201D18]">{label}</strong>
...
function EmptyState({ text }: { text: string }) {
 return <div className="rounded-[6px] border border-dashed border-[#F4EBE6] bg-[#F4EBE6] px-4 py-8 text-center text-sm text-[#6F6A5E]">{text}</div>;
}
