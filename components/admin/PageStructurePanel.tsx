...
export function PageStructurePanel({
 ...
 <aside
 className={cn(
 "admin-structure-panel fixed bottom-5 left-4 top-[76px] z-30 flex w-[304px] flex-col overflow-hidden rounded-[8px] border border-[#DDD6C8] bg-[#FCFAF5]/95 shadow-[0_24px_60px_-30px_rgba(32,29,24,0.35)] backdrop-blur-xl",
 !isDocked && "z-[45]"
 )}
 ...
 <button type="button" onClick={onClose} className="grid h-8 w-8 place-items-center rounded-[4px] border border-[#DDD6C8] bg-[#FCFAF5] text-[#6F6A5E] hover:bg-[#F6F3EC]" aria-label={isZh ? "收起页面结构" : "Close page structure"}>
 <X className="h-4 w-4" />
 </button>
 </div>

 <label className="mt-4 flex h-10 items-center gap-2 rounded-[5px] border border-[#DDD6C8] bg-[#FCFAF5] px-3 text-[#6F6A5E] focus-within:border-[#B23C22] focus-within:ring-4 focus-within:ring-[#B23C22]/15">
 ...
 <div className="mt-3 flex items-center gap-1 rounded-[5px] bg-[#EDE8DB] p-1">
 {(["all", "visible", "hidden"] as VisibilityFilter[]).map((value) => (
 <button
 key={value}
 type="button"
 onClick={() => setVisibility(value)}
 className={cn("flex-1 rounded-[5px] px-2 py-1.5 text-[11px] font-bold transition", visibility === value ? "bg-[#FCFAF5] text-[#201D18]" : "text-[#6F6A5E] hover:text-[#201D18]")}
 >
 ...
 <button type="button" onClick={() => setExpandedIds(new Set(groups.map((group) => group.id)))} className="rounded-[5px] px-2 py-1 text-[11px] font-semibold text-[#6F6A5E] hover:bg-[#FCFAF5]">
 {isZh ? "全部展开" : "Expand all"}
 </button>
 <button type="button" onClick={() => setExpandedIds(new Set())} className="rounded-[5px] px-2 py-1 text-[11px] font-semibold text-[#6F6A5E] hover:bg-[#FCFAF5]">
 ...
 <div className="rounded-[6px] border border-dashed border-[#DDD6C8] px-4 py-10 text-center text-sm text-[#A39C8D]">{isZh ? "没有匹配的模块" : "No matching modules"}</div>
 ) : (
...
function SortableOutlineGroup({
 ...
 className={cn(
 "overflow-hidden rounded-[6px] border bg-[#FCFAF5] transition",
 selected ? "border-[#B23C22] ring-4 ring-[#B23C22]/12" : "border-[#DDD6C8]",
 isDragging && "z-10 opacity-55 shadow-[0_24px_60px_-30px_rgba(32,29,24,0.35)]"
 )}
 >
 ...
 type="button"
 disabled={disabled}
 className="mt-0.5 grid h-8 w-7 shrink-0 touch-none place-items-center rounded-[5px] text-[#A39C8D] hover:bg-[#F6F3EC] hover:text-[#201D18] disabled:cursor-not-allowed disabled:opacity-30"
 ...
 <span className=" text-[10px] font-bold text-[#A39C8D]">{String(index + 1).padStart(2, "0")}</span>
 {group.moduleType ? <span className="rounded-[4px] bg-[#F4EBE6] px-2 py-0.5 text-[9px] font-bold tracking-[0.08em] text-[#B23C22]">{getModuleTypeLabel(group.moduleType, editorLanguage)}</span> : null}
 </div>
 <p className="mt-1 truncate text-sm font-bold text-[#201D18]">{group.title}</p>
 <p className="mt-0.5 text-[10px] text-[#A39C8D]">{isZh ? `${contentCount} 个内容 · ${group.visibleCount}/${group.blocks.length} 显示` : `${contentCount} items · ${group.visibleCount}/${group.blocks.length} visible`}</p>
 </button>
 <div className="flex shrink-0 items-center gap-0.5">
 <button type="button" onClick={onToggleVisibility} className={cn("grid h-8 w-8 place-items-center rounded-[5px] hover:bg-[#F6F3EC]", group.isVisible ? "text-[#B23C22]" : "text-[#A39C8D]")} title={group.isVisible ? (isZh ? "隐藏整个模块" : "Hide module") : (isZh ? "发布整个模块" : "Publish module")}>
 {group.isVisible ? <Eye className="h-4 w-4" /> : <EyeOff className="h-4 w-4" />}
 </button>
 <button type="button" onClick={onEdit} className="grid h-8 w-8 place-items-center rounded-[5px] text-[#6F6A5E] hover:bg-[#F4EBE6] hover:text-[#B23C22]" title={isZh ? "编辑模块" : "Edit module"}>
 <Pencil className="h-4 w-4" />
 </button>
 <button type="button" onClick={onToggleExpanded} className="grid h-8 w-8 place-items-center rounded-[5px] text-[#6F6A5E] hover:bg-[#F6F3EC]" title={expanded ? (isZh ? "收起" : "Collapse") : (isZh ? "展开" : "Expand")}>
 ...
 {expanded ? (
 <div className="border-t border-[#EDE8DB] bg-[#F6F3EC] px-3 py-2.5">
 <div className="grid gap-1">
 {group.blocks.map((block) => (
 <button key={block.id} type="button" onClick={() => onEditBlock(block.id)} className="flex items-center justify-between gap-2 rounded-[5px] px-2 py-1.5 text-left text-[11px] text-[#6F6A5E] hover:bg-[#FCFAF5] hover:text-[#201D18]">
 ...
 {!disabled ? (
 <div className="mt-2 grid grid-cols-4 gap-1 border-t border-[#EDE8DB] pt-2">
...
function MoveButton({ icon, label, disabled, onClick }: { icon: React.ReactNode; label: string; disabled: boolean; onClick: () => void }) {
 return <button type="button" disabled={disabled} onClick={onClick} className="flex flex-col items-center gap-0.5 rounded-[5px] px-1 py-1.5 text-[9px] font-semibold text-[#6F6A5E] hover:bg-[#FCFAF5] disabled:cursor-not-allowed disabled:opacity-30">{icon}{label}</button>;
}
